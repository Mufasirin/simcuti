...
class CetakController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        if($user[0]->ttd == 'qrcode.png'){
             $ttdUserHtml = "<br><br><br><br>";
        }else{
            foreach ($user as $row) {
                $ttdUser = $row->ttd;
            }
             $ttdUserHtml = "<img src='storage/ttd/$ttdUser' alt='tanda Tangan' width='100px'>
                    </td>";
        }
        ...
        if($atasan[0]->ttd == 'qrcode.png'){
             $ttdAtasanHtml = "<br><br><br><br>";
        }else{
            foreach ($atasan as $row) {
                $ttdAtasan = $row->ttd;
            }
             $ttdAtasanHtml = "<img src='storage/ttd/$ttdAtasan' alt='tanda Tangan' width='100px'>";
        }
        foreach ($atasan as $row) {
            $namaAtasan = $row->name;
            $nipAtasan = $row->nip;
